...
class BalanceToken(sp.Contract):
    def __init__(self, _deployer, _metadata):
        self.init_type(t = sp.TRecord(
            deployer = sp.TAddress,
            parent = sp.TAddress,
            metadata = sp.TMap(k = sp.TString, v = sp.TBytes),
            balances = sp.TBigMap(
                k = sp.TAddress,
                v = sp.TRecord(
                    balance = sp.TNat,
    ...
    def mint(self, params):
        sp.set_type(params, sp.TRecord(destination = sp.TAddress, amount = sp.TNat).layout(("destination", "amount")))
    ...
    def burn(self, params):
        sp.set_type(params, sp.TRecord(source = sp.TAddress, amount = sp.TNat).layout(("source", "amount")))
        ...
        sp.if (self.data.balances[params.source].balance == params.amount):
            del self.data.balances[params.source]
        sp.else:
            self.data.balances[params.source].balance = sp.as_nat(self.data.balances[params.source].balance - params.amount)
